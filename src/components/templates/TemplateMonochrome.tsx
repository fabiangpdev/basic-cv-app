  ...
  return (
    <div className="bg-white text-slate-900 p-[45px] min-h-[842px] text-[10px]">
      ...
      {data.projects.length > 0 && (
        <section>
          <h2 className="text-[11px] font-bold uppercase tracking-widest text-slate-900 mb-3">Proyectos</h2>
          <div className="space-y-3">
            {data.projects.map((project) => (
              <div key={project.id}>
                <p className="text-[10px] font-bold text-slate-800">{project.name}</p>
                <p className="text-[9px] text-slate-600">{project.technologies} | {formatDate(project.startDate, project.endDate, project.current)}</p>
