...
export function TemplateCards({ data, lang }: { data: ReturnType<typeof useResumeStore.getState>['resumeData']; lang: Language }) {
    ...
    <div className="bg-white text-slate-900 p-[45px] min-h-[842px] text-[10px]">
      ...
      {data.personalInfo.summary && (
        <section className="mb-6 bg-emerald-50 p-4 rounded-lg border border-emerald-100">
          <h2 className="text-[11px] font-bold text-emerald-800 uppercase mb-2">{L.summary}</h2>
      ...
      {data.experiences.length > 0 && (
        <section className="mb-6">
          <h2 className="text-[12px] font-bold text-emerald-700 uppercase mb-3">{L.experience}</h2>
          <div className="grid grid-cols-2 gap-3">
            {data.experiences.map((exp) => (
              <div key={exp.id} className="bg-white border border-emerald-200 rounded-lg p-3 shadow-sm">
                ...
                <p className="text-[8px] text-slate-400 mt-1">{formatDate(exp.startDate, exp.endDate, exp.current, L.present)}</p>
      ...
      {data.education.length > 0 && (
        <section className="mb-6">
          <h2 className="text-[12px] font-bold text-emerald-700 uppercase mb-3">{L.education}</h2>
          <div className="grid grid-cols-2 gap-3">
            {data.education.map((edu) => (
              <div key={edu.id} className="bg-white border border-emerald-200 rounded-lg p-3 shadow-sm">
                ...
                <p className="text-[8px] text-slate-400 mt-1">{formatDate(edu.startDate, edu.endDate, false, L.present)}</p>
              </div>
            ))}
          </div>
        </section>
      ...
      {data.skills.length > 0 && (
        <section className="mb-6 bg-emerald-50 p-4 rounded-lg border border-emerald-100">
          <h2 className="text-[11px] font-bold text-emerald-800 uppercase mb-3">{L.skills}</h2>
      ...
      {data.projects.length > 0 && (
        <section>
          <h2 className="text-[12px] font-bold text-emerald-700 uppercase mb-3">{L.projects}</h2>
          <div className="grid grid-cols-2 gap-3">
            {data.projects.map((project) => (
              <div key={project.id} className="bg-white border border-emerald-200 rounded-lg p-3 shadow-sm">
                ...
                <p className="text-[8px] text-slate-400 mt-1">{formatDate(project.startDate, project.endDate, project.current, L.present)}</p>
