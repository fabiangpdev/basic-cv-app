      ...
      <div className="grid grid-cols-2 gap-8">
        <div>
          {data.personalInfo.summary && (
            <section className="mb-8 border-l-2 border-blue-500 pl-3">
              <h3 className="text-[11px] font-bold uppercase text-blue-800 mb-2">Resumen</h3>
          ...
          {data.experiences.length > 0 && (
            <section className="mb-8 border-l-2 border-blue-500 pl-3">
              <h3 className="text-[11px] font-bold uppercase text-blue-800 mb-3">Experiencia</h3>
              <div className="space-y-4">
                {data.experiences.map((exp) => (
                  <div key={exp.id}>
                    <p className="text-[11px] font-bold text-slate-800">{exp.position}</p>
                    <p className="text-[10px] text-slate-500 mt-1">{exp.company} | {formatDate(exp.startDate, exp.endDate, exp.current)}</p>
        ...
        <div>
          {data.education.length > 0 && (
            <section className="mb-8 border-l-2 border-blue-500 pl-3">
              <h3 className="text-[11px] font-bold uppercase text-blue-800 mb-3">Educación</h3>
              <div className="space-y-4">
                {data.education.map((edu) => (
                  <div key={edu.id}>
                    <p className="text-[11px] font-bold text-slate-800">{edu.degree} {edu.field && `en ${edu.field}`}</p>
                    <p className="text-[10px] text-slate-500 mt-1">{edu.institution} | {formatDate(edu.startDate, edu.endDate, false)}</p>
                  </div>
                ))}
              </div>
            </section>
          ...
          {data.skills.length > 0 && (
            <section className="border-l-2 border-blue-500 pl-3">
              <h3 className="text-[11px] font-bold uppercase text-blue-800 mb-3">Habilidades</h3>
